...
nk_d3d11_render(ID3D11DeviceContext *context, enum nk_anti_aliasing AA)
{
	...
	context->OMSetDepthStencilState(d3d11.depthState, 1.0f);
...
nk_d3d11_init(ID3D11Device *device, int width, int height, unsigned int max_vertex_buffer, unsigned int max_index_buffer)
{
	...
	//if (!vsName.empty())
	//{
	D3DX11CompileFromFile("Core/UI/nuklear_d3d11.hlsl", NULL, NULL, "vs", "vs_5_0", 0, 0, NULL, &blob, &blobErr, NULL);
		//if (blobErr)
		//{
		...
		device->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &d3d11.vertex_shader);
		...
		device->CreateInputLayout(layout, ARRAYSIZE(layout), blob->GetBufferPointer(), blob->GetBufferSize(), &d3d11.input_layout);
	...
	//if (!psName.empty())
	//{
		D3DX11CompileFromFile("Core/UI/nuklear_d3d11.hlsl", NULL, NULL, "ps", "ps_5_0", 0, 0, NULL, &blob, &blobErr, NULL);
		//if (blobErr)
		//{
		//	std::string err = (char*)blobErr->GetBufferPointer();
		//	OutputDebugStringA(err.c_str());
		...
		device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &d3d11.pixel_shader);
    ...
    NK_ASSERT(SUCCEEDED(hr));}
	{
		...
		device->CreateDepthStencilState(&dsDesc, &d3d11.depthState);
	}
    /* viewport */
